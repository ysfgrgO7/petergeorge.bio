...
export default function StudentsPage() {
  ...
  const fetchStudents = async () => {
    try {
      ...
      const lecturePromises = yearsSnap.docs.map(async (yearDoc) => {
        ...
        return { yearId, snap: await getDoc(lectureRef) };
      });

      const lectureResults = await Promise.all(lecturePromises);

      for (const result of lectureResults) {
        if (result.snap.exists()) {
          foundYearId = result.yearId;
          lectureData = result.snap.data();
          break;
        }
      }
      ...
      // Instead of collectionGroup requiring a composite index, batch fetch specific progress docs chunked
      const progressMap = new Map<string, DocumentData>();
      const chunkSize = 100; // Prevent Firebase quota/network limits over single connection

      for (let i = 0; i < allStudentsSnap.docs.length; i += chunkSize) {
        const chunk = allStudentsSnap.docs.slice(i, i + chunkSize);
        
        const progressPromises = chunk.map((studentDoc) => {
          const studentId = studentDoc.id;
          const progressDocId = `${foundYearId}_${courseId}_${lectureId}`;
          const progressRef = doc(db, "students", studentId, "progress", progressDocId);
          return getDoc(progressRef).then((snap) => ({ studentId, snap }));
        });

        const progressSnaps = await Promise.all(progressPromises);
        
        progressSnaps.forEach(({ studentId, snap }) => {
          if (snap.exists()) {
            progressMap.set(studentId, snap.data());
          }
        });
      }
  ...
  const handleToggleEnabled = async (
    ...
    try {
      ...
      const isSchoolStudent = targetStudent?.system === "school";

      if (
        progressSnap.exists() &&
        (progressSnap.data().unlocked === true || isSchoolStudent)
      ) {
  ...
  const getCompletionStats = () => {
    const totalEnrolled = students.filter(
